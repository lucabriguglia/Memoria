...
- **A recommended indexing policy for the Cosmos DB container.** `CosmosSetup` creates the container with the Cosmos DB default policy, which indexes every path of every document — including the serialised `data` payload, the largest property in the document, which no Memoria query can filter on (`eventPropertyFilter` compiles to `CONTAINS`, which never uses an index). `scripts/install` now ships a policy that indexes only the eight paths the store filters or sorts on, plus three composite indexes for its filter-and-order-by reads, with idempotent PowerShell and Bash scripts to apply it. The package itself is unchanged, so this is opt-in and applies equally to a 1.5.0 deployment. See [Tune the Cosmos DB container](guides/tune-the-cosmos-container.md)
