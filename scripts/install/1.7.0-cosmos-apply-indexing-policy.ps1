<#
.SYNOPSIS
    Applies the Memoria 1.7.0 indexing policy to an Azure Cosmos DB container.
...
.DESCRIPTION
    ...
    1.7.0-cosmos-indexing-policy.json, or recreate the container.
...
.PARAMETER PolicyPath
    Path to the indexing policy JSON. Defaults to 1.7.0-cosmos-indexing-policy.json next to this script.

.PARAMETER Wait
    Poll until the index transformation reports 100% before returning.

.EXAMPLE
    ./1.7.0-cosmos-apply-indexing-policy.ps1 -ResourceGroup rg-shop -Account cosmos-shop -Wait
#>
[CmdletBinding()]
param(
    ...
    [string] $PolicyPath = (Join-Path $PSScriptRoot '1.7.0-cosmos-indexing-policy.json'),
    [switch] $Wait
)
