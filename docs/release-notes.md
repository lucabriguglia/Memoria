...
- **Dynamic consistency boundaries, in two new packages.** `Memoria.EventSourcing.Dcb` and `Memoria.EventSourcing.Dcb.Store.EntityFrameworkCore` add a second consistency model alongside streams. A stream fixes a decision's boundary at design time; under DCB the boundary is a `TagQuery` chosen per decision and evaluated at the moment of the append. Events carry tags, and a boundary selects the events carrying any of them, so one event can fill a seat when read through `course:c1` and use up one of a student's ten when read through `student:s7`. This makes expressible the case no stream holds: a rule spanning a course's capacity *and* a student's course count, where a stream per course cannot see the student's other subscriptions, a stream per student cannot see how full the course is, and one stream for the school serialises every subscription in it. Nothing depends on these packages: installing the streamed store pulls in no DCB, and vice versa. See [Dynamic consistency boundaries](concepts/dynamic-consistency-boundaries.md) and [Streams or DCB?](guides/choose-streams-or-dcb.md)
...
- **`TagQuery` supports `AnyOf` only in this release.** A boundary is a disjunction of tags. Conjunction needs a different query shape and changes which rows an append must lock; the type is shaped so it can be added without a breaking change
...
- **New example.** `examples/Memoria.Examples.EventSourcing.Dcb.EntityFrameworkCore` runs the course-subscription scenario end to end, including two decisions that do not contend and one refused for resting on stale facts
